...
class Saver:
    def __init__(self, args, config, is_adapter, is_ip_adapter, save_root, model, train_dataloader, model_engine, pipeline_model):
        self.args = args
        self.config = config
    ...
    def save_adapter(self, name):
        ...
        print(f'fuccccc {save_dir}')
        # new added
        if self.is_ip_adapter:
            print(f'fuccccc2')
            self.model.save_adapter(save_dir)
            return
