...
class SizeBucketDataset:
    def __init__(self, metadata_dataset, directory_config, size_bucket, cache_base):
        ...
        # === add: id_embeds 相关配置（可选） ===
        self.id_embeds_path = Path(self.directory_config['id_embeds_path']) if 'id_embeds_path' in self.directory_config else None
        if self.id_embeds_path is not None and (not self.id_embeds_path.exists() or not self.id_embeds_path.is_dir()):
            raise RuntimeError(f'Invalid id_embeds_path: {self.id_embeds_path}')
        self.siglip_pooling = bool(self.directory_config.get('siglip_pooling', False))
        # 你也可以按需加：self.fixed_num_tokens / self.pad_trunc 等，用于统一形状
        # =====================================
    ...
    def cache_latents(self, map_fn, regenerate_cache=False, trust_cache=False, caching_batch_size=1):
        ...
        # === add: 在 cache 阶段把 id_embeds 合并进同一个缓存 ===
        # 只有在配置了 id_embeds_path 时才处理；否则跳过保持兼容
        if self.id_embeds_path is not None:
            from datasets import Dataset
            import numpy as np

            cache_file_name = str(self.cache_dir / 'latents_plus_id.arrow')

            def _add_id_embeds(examples):
                # batched=True 时 examples 是列字典（list 对应列），我们逐个样本处理
                out = {'id_embeds': []}
                id_dir = self.id_embeds_path
                use_pool = self.siglip_pooling

                for image_spec in examples['image_spec']:
                    # image_spec = (tar_path_or_None, "path/to/image.png")
                    image_path = Path(image_spec[1])
                    stem = image_path.stem

                    if use_pool:
                        npy_path = id_dir / f"{stem}_wP.npy"
                        pt_path  = id_dir / f"{stem}_wP.pt"
                    else:
                        npy_path = id_dir / f"{stem}.npy"
                        pt_path  = id_dir / f"{stem}.pt"

                    vec = None
                    if npy_path.exists():
                        # 提前用 numpy 读成 float32；cache 为 Arrow，训练期零磁盘读取
                        ...
                        if torch.is_tensor(obj):
                            vec = obj.detach().cpu().numpy().astype(np.float32, copy=False)
                        else:
                            vec = np.array(obj, dtype=np.float32)
                    else:
                        # 若缺失，你可以选择抛错；这里写 None，让后续 __getitem__ 再做判断
                        vec = None

                    # 可选：在此处统一形状（如 pad/trunc 到固定 T），避免 ragged 造成对象列
                    out['id_embeds'].append(vec)

                return out

            # 将 id_embeds 追加到 latent_dataset（带独立缓存文件名）
            self.latent_dataset = self.latent_dataset.map(
                _add_id_embeds,
                batched=True,
                batch_size=caching_batch_size,
                load_from_cache_file=(not regenerate_cache),
            ...
            # 说明：
            # - 这里返回的是 numpy 数组（float32），HF 会把它写进 Arrow。
            # - 若数组形状不统一，HF 会将该列存为“对象列”，仍可用，但吞吐略降。
        # =========================================================
    ...
    def __getitem__(self, idx):
        ...
        # condition_image_latents已经在ret中了
        return ret
...
class DirectoryDataset:
    ...
    def _get_ungrouped_metadata(self, regenerate_cache=False, trust_cache=False):
        ...
        if regenerate_cache or not metadata_cache_file_1.exists() or not trust_cache:
            ...
            # 新逻辑：枚举所有sref文件夹
            sref_folders = [f for f in self.path.glob('*') if f.is_dir()]
            sref_folders.sort()
            
            image_specs = []
            caption_files = []
            control_files = []
            condition_image_files = []  # 新增：存储条件图片
            
            for sref_folder in tqdm(sref_folders):
                sref = sref_folder.name
                # 遍历i=1,2,3,4
                for i in range(1, 5):
                    # 检查必需文件是否存在
                    label_video = sref_folder / f"{sref}-{i}.mp4"
                    condition_video = sref_folder / f"{sref}-{i}-destyle.mp4"
                    caption_file = sref_folder / f"{sref}-{i}-caption.txt"
                    
                    if not (label_video.exists() and condition_video.exists() and caption_file.exists()):
                        logger.warning(f'Missing files for {sref}-{i}, skipping')
                        continue
                    ...
                    for j in range(1, 5):
                        if j != i:
                            img_path = sref_folder / f"{sref}-{j}.png"
                            if img_path.exists():
                                cond_images.append(str(img_path))
                    
                    if len(cond_images) != 3:
                        logger.warning(f'Not enough condition images for {sref}-{i}, skipping')
                        continue
                    
                    # 存储这个训练样本
                    image_specs.append((None, str(label_video)))
                    caption_files.append(str(caption_file))
                    control_files.append(str(condition_video))
                    condition_image_files.append(cond_images)  # 3张图片的列表
            
            d = {
                'image_spec': image_specs, 
                'caption_file': caption_files, 
                'control_file': control_files,
                'condition_images': condition_image_files  # 新增字段
            }
    ...
    def _metadata_map_fn(self):
        ...
        def fn(example):
            ...
            empty_return = {
                'image_spec': [], 'caption': [], 
                'ar_bucket': [], 'size_bucket': [], 'is_video': [],
                'control_file': [], 'condition_images': []
            }
            
            try:
                # 获取视频元数据
                meta = imageio.v3.immeta(str(image_file))
                first_frame = next(imageio.v3.imiter(str(image_file)))
                height, width = first_frame.shape[:2]
                frames = int(self.framerate * meta['duration'])
            except Exception as e:
                logger.warning(f'Media file {image_file} could not be opened. Skipping. {e}')
                return empty_return
            ...
            ret = {
                ...
                'is_video': [is_video],
                'control_file': [example['control_file'][0]],
                'condition_images': [example['condition_images'][0]]  # 传递3张图片
            }
            return ret
...
def _cache_fn(datasets, queue, preprocess_media_file_fn, num_text_encoders, regenerate_cache, trust_cache, caching_batch_size):
    ...
    def latents_map_fn(example, rank):
        ...
        has_condition_images = ('condition_images' in example)
        ...
        for i, (image_spec, size_bucket, caption) in enumerate(
            zip(example['image_spec'], 
                example['size_bucket'], example['caption'])
        ):
            assert size_bucket == first_size_bucket
            
            # 1. 处理label视频
            label_items = preprocess_media_file_fn(image_spec, None, size_bucket)
            assert len(label_items) == 1
            label_tensor, mask = label_items[0]
...
class DatasetManager:
    ...
    def _handle_task(self, task):
        ...
        if id == 0:
            ...
            if len(task) == 5:
                # 有condition_images
                tensor, control_tensor, condition_images, pipe = task[1:]
            else:
                # 旧的兼容逻辑
            ...
            # 调用VAE+SigLIP函数
            results = self.call_vae_fn(tensor, control_tensor, condition_images)
...
class PipelineDataLoader:
    ...
    def load_state_dict(self, state_dict):
        ...
        print(f"Loading dataloader state: epoch {self.epoch}, num_batches_pulled {self.num_batches_pulled}")
        self.num_batches_pulled=self.num_batches_pulled%len(self.dataset)
