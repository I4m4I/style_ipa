...
from transformers import SiglipVisionModel  # 你的环境若是其他类名请相应调整
...
if __name__ == '__main__':
    ...
    if adapter_config := config.get('adapter', None):
        siglip_pooling=adapter_config['siglip_pooling']
        print(f"siglip_pooling:{siglip_pooling}")
    ...
    # new added
    # encode_images_to_npy(folder="/home/zhengtianyu/yaowangzi/laion_high_resolution")
    print("fuck2")
    ...
    if adapter_config := config.get('adapter', None):
        ...
        is_adapter = True
        if adapter_config["type"]=='ip-adapter':  # --- 新增: 支持 IPA ---
            is_ip_adapter = True
    ...
    saver = utils.saver.Saver(args, config, is_adapter, is_ip_adapter, run_dir, model, train_dataloader, model_engine, pipeline_model)
    ...
    # print("DEBUG> dataset_len =", len(train_dataloader.dataset))
    # try:
    #     bs = train_dataloader.batch_size
    # except Exception:
    #     bs = "N/A"
    # print("DEBUG> dataloader_len(steps_per_epoch) =", len(train_dataloader))
    # print("DEBUG> batch_size =", bs)
    # print("DEBUG> grad_accum_steps =", model_engine.gradient_accumulation_steps())
